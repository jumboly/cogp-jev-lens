...
**規模**: 30 ラン・570 リクエスト・入力 478 万 / 出力 74 万トークン（追加実験を含む全体で約 10M トークン・1.5K リクエスト）。断念 0。レイテンシ中央値 約 550 ms / 40 問。
**費用**: Vercel AI Gateway のダッシュボード実測で **$0.00**（2026-09-22 時点、`typesafe-ai/jev` は課金なし）。成功率 65.3%（残りは 429 / 503 の再試行）。
...
## 追加実験: Choice の選択肢を言い換える（v2）

v1（目的地 / 立ち寄り先 / 雰囲気 / 妨げ / 無関係）が Lens の文言と干渉したため、行為の語を避けた
v2（主役 / 脇役 / 背景 / 妨げ / 無関係）で 5 Lens を回した（トークンのみ）。

| Lens | 主役 | 脇役 | 背景 | 妨げ | 無関係 | 平均確信度（v1） |
| --- | --- | --- | --- | --- | --- | --- |
| 子供 | 51 | 158 | 0 | 40 | 495 | 0.60（0.68） |
| 観光客 | 194 | 390 | 2 | 7 | 151 | 0.58（0.64） |
| 静か | 27 | 51 | 31 | 481 | 154 | 0.63（0.65） |
| 散歩 | **76** | 435 | 26 | 40 | 167 | 0.52（0.71） |
| 否定形 | 232 | 68 | 0 | 421 | 23 | 0.61（0.52） |

- **干渉は解消した**: 散歩 Lens で v1 は目的地 0 だったが、v2 では主役 76（公園・カフェ・庭園）
- **妨げ と 主役 は安定**: v1 の妨げ 862 のうち 844 が v2 でも妨げ。目的地 324 のうち 282 が主役
- **脇役 が受け皿になった**: v1 で無関係だった 1,326 のうち 226 が脇役、96 が妨げ、58 が主役に移った。「支えたり補ったり」の定義が緩く、静か Lens では `amenity=toilets`（Score 1.6、やや沈める側）まで脇役に入る
