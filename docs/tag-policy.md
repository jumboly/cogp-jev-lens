...
| 5 | ロングテール | **許可リスト方式。「世界で 100 回以上」または「日本域で 20 回以上」出る `key=value` のみ評価** | タイポ・独自値の排除。正規化辞書・説明文・事前計算キャッシュの鍵を兼ねる。世界基準だけだと日本の中堅チェーンや `cuisine=tempura`（天ぷら）が落ちることが東京駅周辺の抽出で判明（2026-09-22 追記） |
...
`brand` のうち世界で 100 件以上、または日本域で 20 件以上のもの（`brand:ja` / `brand:en` は `brand` の表記違いなので対象外）。
...
- ブランドの表記ゆれ（`brand=7-ELEVEN` / `7-Eleven`、`Docomo Bike Share` / `ドコモ・バイクシェア`）を `brand:wikidata` で束ねる
